...
const FireWall = async (req, res, next) => {
  ...
  try {
    const decoded = VerifyAccessSign(token); // e.g., { sub: userId, role: role, ref: tenantId,tag:["Access","Refresh"] }

    // Check for essential payload properties
    if (!decoded.sub || !decoded.role || !decoded.tag) {
      throw new Error("JWT payload is incomplete.");
    }
    const [role, role_prefix] = decoded.role.split("-");
    if (decoded.tag == "Access") {
      ...
      if (decoded.ref) {
        ...
        if (role_prefix !== "TenantUser") {
          Logger.error(
            functionName,
            "Invalid role suffix for Tenant user path.",
            { role: decoded.role }
          );
          return sendErrorResponse(
            res,
        ...
        if (cachedUser) {
          ...
          Logger.info(
            functionName,
            `🟡 Cache MISS for user ${userId}. Performing DB lookups.`
          );
          ...
          if (!Tenant_user) {
            return sendErrorResponse(
              res,
              {
                statusCode: 401,
                message:
                  "Access Denied. Tenant not found or inactive. Contact your Administrator.",
        ...
        if (role_prefix !== "SystemUser") {
          Logger.error(
            functionName,
            "Invalid role suffix for System user path.",
            { role: decoded.role }
          );
          return sendErrorResponse(
            res,
        ...
        if (!systemUser || !systemUser.isActive) {
          Logger.warn(
            functionName,
            "System User lookup failed or user is inactive.",
            { userId: decoded.sub }
          );
          return sendErrorResponse(
            res,
            {
              statusCode: 401,
              message:
                "System Access Restricted. Account Not Found or Inactive.",
    ...
    } else if (decoded.tag == "Refresh") {
      if (role_prefix == "TenantUser") {
        const user = await User.findOne({ _id: decoded.sub });
        if (!user) {
          Logger.warn(
            functionName,
            "Tenant User lookup failed or user is inactive.",
            { userId: decoded.sub }
          );
          return sendErrorResponse(
            res,
        ...
        req.user = user;
        return next();
      } else if (role_prefix == "SystemUser") {
        const user = await SystemUser.findOne({ _id: decoded.sub });
        if (!user) {
          Logger.warn(
            functionName,
            "System User lookup failed or user is inactive.",
    ...
    Logger.error(
      "FireWall",
      "JWT verification or database access failed.",
      error
    );
    ...
    const isJwtError =
      error.name &&
      (error.name.includes("Token") || error.name.includes("JWT"));
